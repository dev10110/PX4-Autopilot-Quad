...
void flat_state_ENU_to_quad_state_ENU(Vector3f& b1d, Vector3f& ang_vel, Vector3f& ang_acc, 
  ...
  float g=9.81f
    ...
    Vector3f Sd (
        ...
        (bx1 * w1) / (bx12 + bx22) +
        (bx2 * w2) / (bx12 + bx22) +
        ((bx12 * bz1 - bx22 * bz1 + 2 * bx1 * bx2 * bz2) * w3) / _sq(bx12 + bx22),
        ((bx12 * bx2 + _cube(bx2) - bx12 * by1 + bx22 * by1 - 2 * bx1 * bx2 * by2) * w3) / _sq(bx12 + bx22)
    );
...
void flat_state_FRD_to_quad_state_FRD(
  ...
  const Vector3f acc_FRD, 
  const Vector3f jerk_FRD,
  const Vector3f snap_FRD, 
  float yaw, 
  float yaw_rate, 
  float yaw_acc, 
  float g=9.81f
  ...
  // allocate temporary matrices
	...
	flat_state_ENU_to_quad_state_ENU(b1d_ENU, ang_vel_ENU, ang_acc_ENU, acc_ENU, jerk_ENU, snap_ENU, yaw + (float)M_PI/2.0f, yaw_rate, yaw_acc, g);
